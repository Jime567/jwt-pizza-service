...
describe('Database Tests', () => {
  ...
  beforeAll(async () => {
    // Create a user to use in the franchise tests
    user = { name: 'Test User', email: 'a@jwt.com', password: 'password123', roles: [{ role: Role.Admin }] };
    await DB.addUser(user);
  });

  it('should add a user', async () => {
    const newUser = { name: 'test user', email: 'test@example.com', password: 'password123', roles: [{ role: Role.Diner }] };
    const result = await DB.addUser(newUser);
    expect(result).toMatchObject({ name: newUser.name, email: newUser.email });
  });
